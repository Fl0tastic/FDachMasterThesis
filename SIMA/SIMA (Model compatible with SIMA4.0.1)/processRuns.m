...
%% Loadnames
allcases1=['6A';'6B';'6C';'6G';'6H';'6I';'6J';'6K';'6L';'8A';'8B';'8C';'8G';'8H';'8I';'8J';'8K';'8L'];
allcases2=['10A';'10B';'10C';'10G';'10H';'10I';'10J';'10K';'10L';'12A';'12B';'12C';'12G';'12H';'12I';'12J';'12K';'12L';'14A';'14B';'14C';'14G';'14H';'14I';'14J';'14K';'14L'];
%%

for b=1:45 %if all cases should be processed
    if b<19
        x=allcases1(b,:);
    else
        x=allcases2((b-18),:);
    ...
    for j=1
        ...
        for q=1
            ...
            yBR=cfreq_BR(find(BRoutcr==nbcr_BR));
            yH=cfreq_H(find(Houtcr==nbcr_H));
            yD=cfreq_D(find(Doutcr==nbcr_D));
            BRleg=['Blade-root \eta_b= ' num2str(nbcr_BR) 'm'];
            Hleg=['Hub \eta_h= ' num2str(nbcr_H) 'm'];
            Rleg=['Relative motion \eta_r= ' num2str(nbcr_D) 'm'];
            nctit=['Variation in the outcrossing rate with motion radius for LC ' x ' 50mm rope'];

            %         figure()
            %         plot(BRoutcr,cfreq_BR,"k.", 'HandleVisibility','off'); hold on
            %         grid on
            %         grid minor
            %         plot(Houtcr,cfreq_H,"k.", 'HandleVisibility','off');
            %         plot(Doutcr,cfreq_D,"k.", 'HandleVisibility','off');
            %         pBR=plot(nbcr_BR,yBR,'Color','r','Marker','o', 'DisplayName', BRleg);
            %         pH=plot(nbcr_H,yH,'Color','r','Marker','^', 'DisplayName', Hleg);
            %         pD=plot(nbcr_D,yD,'Color','r','Marker','square', 'DisplayName', Rleg);
    ...
    %% Pin & Flange
    Fs = 36000 / 1800;
    cutoffFrequency = 0.3; % Set the cutoff frequency (in Hz)
...
for i=1
    ...
    %     figure()
    %     plot(fpsd_B,psd_B,'Color','k'), hold on, plot(fpsd_H,psd_H,'Color','r'),
    %     xlabel('Frequency [Hz]'), ylabel('Spectral density of motion radius [m^2/Hz]'), grid on,
    %     xlim([0 0.5]), legend('Blade root','Hub')
    %
    %     figure()
    %     subplot(2,2,1)
    %     plot(timeRif,BladeRadius_L1,'Color','k'), hold on, xlabel('time[s]'), ylabel('Blade motion radius [m]'),grid on, xlim([0 1800])
    %
    %     subplot(2,2,2)
    %     plot(fpsd_B,psd_B,'Color','k'), hold on,
    %     xlabel('Frequency [Hz]'), ylabel('Spectral density of motion radius [m^2/Hz]'), grid on,
    %     xlim([0 0.2])
    %
    %     subplot(2,2,3)
    %     plot(timeRif,HubRadius_L1,'Color','r'), hold on, xlabel('time[s]'), ylabel('Hub motion radius [m]'),grid on, xlim([0 1800])
    %
    %     subplot(2,2,4)
    %     plot(fpsd_H,psd_H,'Color','r'), hold on,
    %     xlabel('Frequency [Hz]'), ylabel('Spectral density of motion radius [m^2/Hz]'), grid on,
    %     xlim([0 0.2])
    %     sgtitle('Comparison of blade and hub motion radius')

    %blade psds
    %     figure()
    %     subplot(3,1,1)
    %     plot(fpsd_B_Su,psd_B_Su), hold on,
    ...
    %
    %     subplot(3,1,2)
    %     plot(fpsd_B_Sw,psd_B_Sw), hold on,
    ...
    %
    %     subplot(3,1,3)
    %     plot(fpsd_B_Yw,psd_B_Yw), hold on,
    ...
    %     figure()
    %     subplot(2,1,1)
    %     plot(fpsd_BR_X,psd_BR_X), hold on,
    ...
    %
    %     subplot(2,1,2)
    %     plot(fpsd_BR_Z, psd_BR_Z), hold on,
    ...
    %     NatPerlab=['Natural period= ' NatPer 's'];
    %     legend('PSD',NatPerlab)
    %     sgtitle('Mean psds of hub motion')
end
...
for i=1
    ...
    %     figure()
    %     subplot(3,1,1)
    %     plot(fpsd_Su,psd_Su,'Color','k'), hold on,
    ...
    %
    %     subplot(3,1,2)
    %     plot(fpsd_Sw,psd_Sw,'Color','k'), hold on,
    ...
    %
    %     subplot(3,1,3)
    %     plot(fpsd_He,psd_He,'Color','k'), hold on,
